...
from re import search as regex_search, Match
from unicodedata import normalize

from breg.config.config import Configuration
from breg.core.network.net import Session, ContentType
...
class Fetcher:
    ...
    def fetch_enrollment_data(
        ...
        response = self._session.post(
            path=self._config.TARGET_FETCH_ENROLLMENTS_PATH,
            body=b"",
        )
        ...
        return _parse_enrollment_data(response_data)

    def fetch_course_data(self, search_string: str) -> list[CourseCache]:
        response = self._session.post(
            path=self._config.TARGET_FETCH_COURSES_PATH,
            body=f"{self._config.COURSE_CODE_NAME}={search_string}".encode(),
        ...
        response_data = response.read().decode()
        return _parse_course_data(response_data)

    def fetch_class_data(self, course_id: str) -> list[ClassCache]:
        response = self._session.post(
            path=self._config.TARGET_FETCH_CLASSES_PATH,
            body=f"{self._config.COURSE_ID_NAME}={course_id}".encode(),
            content_type=ContentType.FORM_URLENCODED,
        )
        ...
        return _parse_class_data(response_data)

    def _check_net_error(self, response, fail_message) -> None:
...
def _parse_enrollment_data(
    ...
    for panel in panels:
        ...
        # 0: Header
        # 1: info
        class_cache = _parse_class_info(panel_body_content[1])
        class_cache.course_code = course_code
        class_cache.schedules = _parse_schedule_table(
...
def _parse_course_data(raw_data: str) -> list[CourseCache]:
    soup = BeautifulSoup(raw_data, "html.parser")
...
def _parse_class_data(raw_data: str) -> list[ClassCache]:
    soup = BeautifulSoup(raw_data, "html.parser")
    ...
    for row in rows:
        ...
        if row.select_one("table"):
            ...
            last_class.schedules.extend(_parse_schedule_table(row.select_one("table")))
            continue

        classes.append(_parse_class_info(row))

    return classes


def _parse_schedule_table(table: Tag) -> list[Schedule]:
...
def _parse_class_info(row: Tag) -> ClassCache:
